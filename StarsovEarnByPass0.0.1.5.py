...
class StarsovEarnByPass(loader.Module):
        ...
        try:
            ...
            if entity_info.__class__.__name__ == 'Channel':
                ...
                try:
                    ...
                    await message_my.edit(f"Отправил /start боту: {entity_info.username}")
                    return True
                except:
                    await message_my.edit("Не удалось отправить сообщение - скипаю")
                    return False
    ...
    def extract_and_replace_fruit_emojis(self, text):
        ...
        for i, word in enumerate(words):
            ...
            emojized = emoji.emojize(emoji_code)
    ...
    async def on_message(self, message):  
        ...
        if "дождитесь прогрузки ссылки" in message.text:
        ...
        if "Проверка на робота" in message.text:
            ...
            row, _ = await self.find_button_by_text(emoji_find[0], message)
            if not row: return
